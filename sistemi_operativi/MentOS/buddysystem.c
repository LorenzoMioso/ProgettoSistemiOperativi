...
block_found:
	...
	unsigned int size = 1LU << current_order;
	while (current_order > order) {
		...
		page_t *buddy = (page_t *)get_buddy_idx(size, current_order);

		// set the order of pages after the buddy page_t (the field
		// 'private')
		buddy->private = current_order - 1;
		...
		list_head_add(&buddy->lru, area->free_list.next);
...
void bb_free_pages(zone_t *zone, page_t *page, unsigned int order)
{
	...
	list_head_add(&coalesced->lru, zone->free_area[order].free_list.next);
