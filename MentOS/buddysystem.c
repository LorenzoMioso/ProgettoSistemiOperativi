...
block_found:
    ...
    page = list_entry(&area->free_list);
    ...
    while (current_order > order) {
        ...
        page_t *buddy = get_buddy_idx(size, current_order);
...
void bb_free_pages(zone_t *zone, page_t *page, unsigned int order) {
    ...
    while (order < MAX_ORDER - 1) {
        ...
        if (!(buddy->_count == -1 && pages->private == buddy->private)) {
            break;
        }
